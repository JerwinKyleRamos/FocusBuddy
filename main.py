...
while True:
    ...
    if landmarks_points:
        ...
        def gaze_ratio_vertical(top, bottom, iris):
        ...
        left_h = gaze_ratio_horizontal(left_outer, left_inner, left_pupil_center)
        right_h = gaze_ratio_horizontal(right_inner, right_outer, right_pupil_center)
        avg_h = (left_h + right_h) / 2

        # Vertical iris ratios
        left_v = gaze_ratio_vertical(left_upper,left_lower, left_pupil_center,)
        right_v = gaze_ratio_vertical(right_upper, right_lower, right_pupil_center)
        ...
        # cv2.circle(frame, tuple(left_pupil_center), 4, (0, 255, 255), -1)
        ...
        if not is_looking_at_screen:
            ...
            # cv2.putText(frame, f"AWAY TIMER: {elapsed:.1f}s", (30, 90),
            #             cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 0), 2)
            if elapsed >= start_alarm and not alarm_on:
